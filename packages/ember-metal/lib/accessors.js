...
Ember.getPath = function(root, path, _checkGlobal) {
  ...
  if (!root || hasThis || hasStar) {
    Ember.deprecate("Fetching globals with Ember.getPath is deprecated (root: "+root+", path: "+path+")", !root || root === ROOT || !IS_GLOBAL.test(path));
...
Ember.setPath = function(root, path, value, tolerant) {
  ...
  path = normalizePath(path);
  if (path.indexOf('*')>0) {
    Ember.deprecate("Setting globals with Ember.setPath is deprecated (path: "+path+")", !root || root === ROOT || !IS_GLOBAL.test(path));
    var tuple = normalizeTuple(root, path);
    root = tuple[0];
    path = tuple[1];
    tuple.length = 0;
